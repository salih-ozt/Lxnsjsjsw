...
async function initializeDatabase() {
    ...
    // ════════════════════════════════════════════════════════════════════
    // 💬 CHATAPP — Ayrı tablolar (mevcut tablolarla çakışmaz)
    // ════════════════════════════════════════════════════════════════════
    await pool.query(`
    ...
    await pool.query(`
        CREATE TABLE IF NOT EXISTS chat_messages (
            id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
            "senderId"          UUID NOT NULL REFERENCES chat_users(id) ON DELETE CASCADE,
            "recipientId"       UUID NOT NULL REFERENCES chat_users(id) ON DELETE CASCADE,
            "senderUsername"    TEXT NOT NULL,
            "recipientUsername" TEXT NOT NULL,
            content             TEXT NOT NULL,
            "mediaUrl"          TEXT,
            ...
            "deletedByRecip"    BOOLEAN DEFAULT FALSE,
            "createdAt"         TIMESTAMPTZ NOT NULL DEFAULT NOW(),
            "updatedAt"         TIMESTAMPTZ NOT NULL DEFAULT NOW()
        )
    `).catch(() => {});
    ...
    await pool.query(`
        CREATE TABLE IF NOT EXISTS chat_blocks (
            id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
            "blockerId" UUID NOT NULL REFERENCES chat_users(id) ON DELETE CASCADE,
            "blockedId" UUID NOT NULL REFERENCES chat_users(id) ON DELETE CASCADE,
            "createdAt" TIMESTAMPTZ NOT NULL DEFAULT NOW(),
            UNIQUE("blockerId", "blockedId")
        )
    `).catch(() => {});
    ...
    await pool.query(`
        CREATE TABLE IF NOT EXISTS chat_refresh_tokens (
            id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
            "userId"    UUID NOT NULL REFERENCES chat_users(id) ON DELETE CASCADE,
    ...
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_chat_users_username  ON chat_users(username)`).catch(() => {});
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_chat_users_email     ON chat_users(email)`).catch(() => {});
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_chat_msg_sender      ON chat_messages("senderId")`).catch(() => {});
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_chat_msg_recipient   ON chat_messages("recipientId")`).catch(() => {});
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_chat_msg_created     ON chat_messages("createdAt" DESC)`).catch(() => {});
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_chat_blocks_blocker  ON chat_blocks("blockerId")`).catch(() => {});
    console.log('✅ ChatApp tabloları hazır (chat_users, chat_messages, chat_blocks)');
...
// =============================================================================
// 💬 CHATAPP — Prefix: /chat  (diğer API'lerle kesinlikle çakışmaz)
// Tablolar: chat_users, chat_messages, chat_blocks, chat_refresh_tokens
// Frontend: public/chat/index.html  → https://sehitumitkestitarimmtal.com/chat/
// =============================================================================

// ── Statik dosya servisi (/chat/ → public/chat/) ──────────────────────────
const chatDir = path.join(__dirname, 'public', 'chat');
if (fssync.existsSync(chatDir)) {
    app.use('/chat', express.static(chatDir, { maxAge: '1d', index: 'index.html' }));
}

// SPA fallback — /chat/* → index.html
app.get('/chat', (req, res) => res.redirect('/chat/'));
app.get('/chat/', (req, res) => {
    const p = path.join(__dirname, 'public', 'chat', 'index.html');
    fssync.existsSync(p)
        ? res.sendFile(p)
        : res.status(404).send('<h2>ChatApp bulunamadi.<br>public/chat/index.html dosyasini ekleyin.</h2>');
});
app.get('/chat/*', (req, res) => {
    if (req.path.startsWith('/chat/api/')) return res.status(404).json({ error: 'Rota bulunamadi' });
    const p = path.join(__dirname, 'public', 'chat', 'index.html');
    fssync.existsSync(p) ? res.sendFile(p) : res.status(404).json({ error: 'Sayfa bulunamadi' });
});

// ── Rate limiting — /chat/api/* için ayrı limitler ──────────────────────────
const chatAuthLimiter    = rateLimit({ windowMs: 15*60*1000, max: 10,  message: { error: 'Cok fazla istek. 15 dk bekleyin.' }, skip: r => r.method === 'OPTIONS' });
const chatMsgLimiter     = rateLimit({ windowMs:    60*1000, max: 60,  message: { error: 'Mesaj limiti asildi.' } });
const chatSearchLimiter  = rateLimit({ windowMs:    60*1000, max: 30,  message: { error: 'Arama limiti asildi.' } });

// ── ChatApp JWT dogrulamasi (mevcut authenticateToken'dan bagimsiz) ─────────
async function chatAuthMiddleware(req, res, next) {
    let token = req.cookies?.chat_access_token;
    if (!token) {
    ...
    try {
        const decoded = jwt.verify(token, JWT_SECRET);
        const user = await dbGet(
            `SELECT id, username, name, email, "profilePic", bio, "isOnline", "lastSeen", "isActive", "isBanned"
             FROM chat_users WHERE id = $1 AND "isActive" = TRUE`,
            [decoded.id]
        );
        if (!user) return res.status(403).json({ error: 'Kullanici bulunamadi' });
        if (user.isBanned) return res.status(403).json({ error: 'Hesabiniz yasaklandi' });
        req.chatUser = user;
        next();
    } catch (e) {
...
// ── ChatApp token uretici ────────────────────────────────────────────────────
function chatGenerateTokens(user) {
    const accessToken = jwt.sign(
        { id: user.id, email: user.email, username: user.username, type: 'chat' },
        JWT_SECRET,
        { expiresIn: '1d', algorithm: 'HS256' }
    );
    const refreshToken = jwt.sign(
        { id: user.id, type: 'chat_refresh' },
        JWT_REFRESH_SECRET,
        { expiresIn: '30d', algorithm: 'HS256' }
    );
    return { accessToken, refreshToken };
...
// =============================================================================
// CHATAPP AUTH ROTALARI — /chat/api/auth/*
// =============================================================================

// POST /chat/api/auth/register
app.post('/chat/api/auth/register', chatAuthLimiter, async (req, res) => {
    try {
        ...
        if (!cleanUsername || cleanUsername.length < 3)
            return res.status(400).json({ error: 'Kullanici adi gecersiz (en az 3 karakter, a-z 0-9 . _ -)' });

        const existing = await dbGet(
            'SELECT id FROM chat_users WHERE username = $1 OR email = $2',
            [cleanUsername, cleanEmail]
        );
        if (existing) return res.status(400).json({ error: 'Bu kullanici adi veya e-posta zaten kayitli' });

        const hashed = await bcrypt.hash(password, 10);
        const userId = uuidv4();

        await dbRun(
            `INSERT INTO chat_users (id, name, username, email, password, "createdAt", "updatedAt")
             VALUES ($1, $2, $3, $4, $5, NOW(), NOW())`,
            [userId, name.trim().substring(0, 60), cleanUsername, cleanEmail, hashed]
        );

        const tokens = chatGenerateTokens({ id: userId, email: cleanEmail, username: cleanUsername });

        const tokenHash = crypto.createHash('sha256').update(tokens.refreshToken).digest('hex');
        await dbRun(
            `INSERT INTO chat_refresh_tokens (id, "userId", "tokenHash", ip, "userAgent", "createdAt", "expiresAt")
             VALUES ($1, $2, $3, $4, $5, NOW(), NOW() + INTERVAL '30 days')`,
            [uuidv4(), userId, tokenHash, req.ip, req.headers['user-agent'] || '']
        );

        const isSecure = req.secure || req.headers['x-forwarded-proto'] === 'https';
        res.cookie('chat_access_token', tokens.accessToken, {
            httpOnly: true, secure: isSecure, sameSite: 'strict', maxAge: 86400000
        });
        ...
        res.status(201).json({
            ...
            token: tokens.accessToken,
            refreshToken: tokens.refreshToken,
            user: { id: userId, username: cleanUsername, name: name.trim(), email: cleanEmail, profilePic: null }
        });
    } catch (err) {
        console.error('[ChatApp Register]', err.message);
        res.status(500).json({ error: 'Sunucu hatasi' });
    }
});

// POST /chat/api/auth/login
app.post('/chat/api/auth/login', chatAuthLimiter, async (req, res) => {
    try {
        const { identifier, email, password } = req.body;
        const loginId = ((identifier || email || '')).toLowerCase().trim();
        if (!loginId || !password)
            return res.status(400).json({ error: 'E-posta/kullanici adi ve sifre gerekli' });

        const user = await dbGet(
            `SELECT * FROM chat_users WHERE (email = $1 OR username = $1) AND "isActive" = TRUE`,
            [loginId]
        );
        ...
        const valid = await bcrypt.compare(password, user.password);
        if (!valid) return res.status(401).json({ error: 'Sifre yanlis' });

        await dbRun(
            `UPDATE chat_users SET "isOnline" = TRUE, "lastSeen" = NOW(), "updatedAt" = NOW() WHERE id = $1`,
            [user.id]
        );

        const tokens = chatGenerateTokens(user);
        const tokenHash = crypto.createHash('sha256').update(tokens.refreshToken).digest('hex');
        await dbRun(
            `INSERT INTO chat_refresh_tokens (id, "userId", "tokenHash", ip, "userAgent", "createdAt", "expiresAt")
             VALUES ($1, $2, $3, $4, $5, NOW(), NOW() + INTERVAL '30 days')
        ...
        res.json({
            ...
            user: {
                ...
                email: user.email, profilePic: user.profilePic, bio: user.bio
            }
        });
    } catch (err) {
        console.error('[ChatApp Login]', err.message);
        res.status(500).json({ error: 'Sunucu hatasi' });
    }
});

// POST /chat/api/auth/logout
app.post('/chat/api/auth/logout', chatAuthMiddleware, async (req, res) => {
    try {
        await dbRun(
            `UPDATE chat_users SET "isOnline" = FALSE, "lastSeen" = NOW(), "updatedAt" = NOW() WHERE id = $1`,
            [req.chatUser.id]
        );
        await dbRun(`DELETE FROM chat_refresh_tokens WHERE "userId" = $1`, [req.chatUser.id]);
...
// POST /chat/api/auth/refresh
app.post('/chat/api/auth/refresh', async (req, res) => {
    try {
        const rt = req.body.refreshToken || req.cookies?.chat_refresh_token;
        if (!rt) return res.status(401).json({ error: 'Refresh token gerekli' });

        const decoded = jwt.verify(rt, JWT_REFRESH_SECRET);
        if (decoded.type !== 'chat_refresh') return res.status(403).json({ error: 'Gecersiz token tipi' });

        const hash = crypto.createHash('sha256').update(rt).digest('hex');
        const stored = await dbGet(
            `SELECT * FROM chat_refresh_tokens WHERE "tokenHash" = $1 AND "isActive" = TRUE AND "expiresAt" > NOW()`,
            [hash]
        );
        if (!stored) return res.status(403).json({ error: 'Token gecersiz veya suresi dolmus' });

        const user = await dbGet(`SELECT * FROM chat_users WHERE id = $1 AND "isActive" = TRUE`, [decoded.id]);
        if (!user) return res.status(403).json({ error: 'Kullanici bulunamadi' });

        const tokens = chatGenerateTokens(user);
        const newHash = crypto.createHash('sha256').update(tokens.refreshToken).digest('hex');

        await dbRun(`UPDATE chat_refresh_tokens SET "isActive" = FALSE WHERE "tokenHash" = $1`, [hash]);
        await dbRun(
            `INSERT INTO chat_refresh_tokens (id, "userId", "tokenHash", ip, "userAgent", "createdAt", "expiresAt")
             VALUES ($1, $2, $3, $4, $5, NOW(), NOW() + INTERVAL '30 days')`,
            [uuidv4(), user.id, newHash, req.ip, req.headers['user-agent'] || '']
...
// GET /chat/api/auth/me
app.get('/chat/api/auth/me', chatAuthMiddleware, (req, res) => {
    const u = req.chatUser;
    res.json({ user: { id: u.id, username: u.username, name: u.name, email: u.email, profilePic: u.profilePic, bio: u.bio, isOnline: u.isOnline, lastSeen: u.lastSeen } });
});

// =============================================================================
// CHATAPP KULLANICI ROTALARI — /chat/api/users/*
// =============================================================================

// GET /chat/api/users/search — query param ?q= veya /:query compat
app.get('/chat/api/users/search', chatAuthMiddleware, chatSearchLimiter, async (req, res) => {
    try {
        const query = (req.query.q || '').trim();
        if (!query || query.length < 2) return res.json({ users: [] });
        const q = '%' + query.replace(/[%_]/g, '\\$&') + '%';
        const users = await dbAll(
            `SELECT id, username, name, "profilePic", "isOnline", "lastSeen"
             FROM chat_users
             WHERE (username ILIKE $1 OR name ILIKE $1) AND "isActive" = TRUE AND id != $2
             ORDER BY username LIMIT 20`,
            [q, req.chatUser.id]
        );
        const blocked = await dbAll(
            `SELECT "blockedId" FROM chat_blocks WHERE "blockerId" = $1
             UNION SELECT "blockerId" FROM chat_blocks WHERE "blockedId" = $1`,
            [req.chatUser.id]
        );
        const blockedIds = new Set(blocked.map(b => b.blockedId || b.blockerId));
        res.json({ users: users.filter(u => !blockedIds.has(u.id)) });
    } catch (err) {
        res.status(500).json({ error: 'Sunucu hatasi' });
    }
});

// GET /chat/api/users/search/:query — eski compat
app.get('/chat/api/users/search/:query', chatAuthMiddleware, chatSearchLimiter, async (req, res) => {
    try {
        const q = '%' + req.params.query.trim().replace(/[%_]/g, '\\$&') + '%';
        const users = await dbAll(
            `SELECT id, username, name, "profilePic", "isOnline", "lastSeen"
             FROM chat_users
             WHERE (username ILIKE $1 OR name ILIKE $1) AND "isActive" = TRUE AND id != $2
...
// GET /chat/api/users/:id — Kullanici profili (isBlocked dahil)
app.get('/chat/api/users/:id', chatAuthMiddleware, async (req, res) => {
    try {
        const user = await dbGet(
            `SELECT id, username, name, "profilePic", bio, "isOnline", "lastSeen", "createdAt"
             FROM chat_users WHERE id = $1 AND "isActive" = TRUE`,
            [req.params.id]
        );
        ...
        const block = await dbGet(
            `SELECT id FROM chat_blocks WHERE "blockerId" = $1 AND "blockedId" = $2`,
            [req.chatUser.id, req.params.id]
        );
        res.json({ user: { ...user, isBlocked: !!block } });
    } catch (err) {
        res.status(500).json({ error: 'Sunucu hatasi' });
    }
});

// PUT /chat/api/users/profile — Profil guncelle
app.put('/chat/api/users/profile', chatAuthMiddleware, async (req, res) => {
    try {
        const { name, bio } = req.body;
        if (!name || !name.trim()) return res.status(400).json({ error: 'Ad gerekli' });
        await dbRun(
            `UPDATE chat_users SET name = $1, bio = $2, "updatedAt" = NOW() WHERE id = $3`,
            [name.trim().substring(0, 60), (bio || '').substring(0, 200), req.chatUser.id]
        );
...
// POST /chat/api/users/:id/block — Kullanici engelle
app.post('/chat/api/users/:id/block', chatAuthMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        if (id === req.chatUser.id) return res.status(400).json({ error: 'Kendinizi engelleyemezsiniz' });
        const target = await dbGet('SELECT id FROM chat_users WHERE id = $1', [id]);
        if (!target) return res.status(404).json({ error: 'Kullanici bulunamadi' });
        await dbRun(
            `INSERT INTO chat_blocks ("id","blockerId","blockedId","createdAt") VALUES ($1,$2,$3,NOW()) ON CONFLICT DO NOTHING`,
            [uuidv4(), req.chatUser.id, id]
        );
        res.json({ message: 'Kullanici engellendi' });
    } catch (err) {
        res.status(500).json({ error: 'Sunucu hatasi' });
    }
});

// DELETE /chat/api/users/:id/block — Engeli kaldir
app.delete('/chat/api/users/:id/block', chatAuthMiddleware, async (req, res) => {
    try {
        await dbRun(
            `DELETE FROM chat_blocks WHERE "blockerId" = $1 AND "blockedId" = $2`,
            [req.chatUser.id, req.params.id]
        );
        res.json({ message: 'Engel kaldirildi' });
    } catch (err) {
        res.status(500).json({ error: 'Sunucu hatasi' });
    }
});

// DELETE /chat/api/users/:id/unblock — Alias
app.delete('/chat/api/users/:id/unblock', chatAuthMiddleware, async (req, res) => {
    try {
        await dbRun(`DELETE FROM chat_blocks WHERE "blockerId"=$1 AND "blockedId"=$2`, [req.chatUser.id, req.params.id]);
        res.json({ message: 'Engel kaldirildi' });
    } catch (err) { res.status(500).json({ error: 'Sunucu hatasi' }); }
});

// GET /chat/api/users/blocks — Engellenenler listesi
app.get('/chat/api/users/blocks', chatAuthMiddleware, async (req, res) => {
    try {
        const list = await dbAll(
            `SELECT cu.id, cu.username, cu.name, cu."profilePic", cb."createdAt" as "blockedAt"
             FROM chat_blocks cb JOIN chat_users cu ON cb."blockedId" = cu.id
             WHERE cb."blockerId" = $1 ORDER BY cb."createdAt" DESC`,
            [req.chatUser.id]
        );
...
// =============================================================================
// CHATAPP MESAJ ROTALARI — /chat/api/messages/*
// =============================================================================

// POST /chat/api/messages — Mesaj gonder (HTTP fallback)
app.post('/chat/api/messages', chatAuthMiddleware, chatMsgLimiter, async (req, res) => {
    try {
        const { recipientId, content } = req.body;
        if (!recipientId || !content || !content.trim())
            return res.status(400).json({ error: 'Alici ve mesaj gerekli' });
        if (content.length > 5000) return res.status(400).json({ error: 'Mesaj cok uzun (max 5000 karakter)' });

        // Engel kontrolu
        const blocked = await dbGet(
            `SELECT id FROM chat_blocks WHERE ("blockerId"=$1 AND "blockedId"=$2) OR ("blockerId"=$2 AND "blockedId"=$1)`,
            [req.chatUser.id, recipientId]
        );
        if (blocked) return res.status(403).json({ error: 'Bu kullaniciya mesaj gonderemezsiniz' });

        const recipient = await dbGet(
            `SELECT id, username FROM chat_users WHERE id = $1 AND "isActive" = TRUE`,
            [recipientId]
        );
        ...
        const msgId = uuidv4();
        await dbRun(
            `INSERT INTO chat_messages (id,"senderId","recipientId","senderUsername","recipientUsername",content,"createdAt","updatedAt")
             VALUES ($1,$2,$3,$4,$5,$6,NOW(),NOW())`,
            [msgId, req.chatUser.id, recipientId, req.chatUser.username, recipient.username, content.trim()]
        );

        const msg = await dbGet(`SELECT * FROM chat_messages WHERE id = $1`, [msgId]);

        // Socket ile gercek zamanli ilet (socket.io'daki onlineUsers haritasi paylasiliyor)
        if (io && onlineUsers.has(recipientId)) {
            for (const sid of onlineUsers.get(recipientId)) {
                io.to(sid).emit('chat:message:receive', msg);
            }
        }

        res.status(201).json({ message: 'Mesaj gonderildi', msg });
    } catch (err) {
        console.error('[ChatApp SendMsg]', err.message);
        res.status(500).json({ error: 'Sunucu hatasi' });
    }
});

// GET /chat/api/messages/conversations — Sohbet listesi
app.get('/chat/api/messages/conversations', chatAuthMiddleware, async (req, res) => {
    try {
        const rows = await dbAll(
            `SELECT DISTINCT ON (partner_id)
                    m.*,
             ...
             WHERE (m."senderId" = $1 AND m."deletedBySender" = FALSE)
                OR (m."recipientId" = $1 AND m."deletedByRecip" = FALSE)
             ORDER BY partner_id, m."createdAt" DESC`,
            [req.chatUser.id]
        );

        const convs = await Promise.all(rows.map(async (row) => {
            const partner = await dbGet(
                `SELECT id, username, name, "profilePic", "isOnline", "lastSeen" FROM chat_users WHERE id = $1`,
                [row.partner_id]
            );
            const unread = await dbGet(
                `SELECT COUNT(*)::int AS count FROM chat_messages
                 WHERE "senderId" = $1 AND "recipientId" = $2 AND "isRead" = FALSE AND "deletedByRecip" = FALSE`,
                [row.partner_id, req.chatUser.id]
            );
            return { ...row, partner, unreadCount: unread?.count || 0 };
        }));

        res.json({ conversations: convs });
    } catch (err) {
        console.error('[ChatApp Conversations]', err.message);
        res.status(500).json({ error: 'Sunucu hatasi' });
    }
});

// GET /chat/api/messages/:userId — Mesaj gecmisi
app.get('/chat/api/messages/:userId', chatAuthMiddleware, async (req, res) => {
    try {
        const { page = 1, limit = 50 } = req.query;
        const pg = Math.max(1, parseInt(page) || 1);
        const lm = Math.min(parseInt(limit) || 50, 100);
        const offset = (pg - 1) * lm;

        const messages = await dbAll(
            `SELECT * FROM chat_messages
             WHERE (("senderId"=$1 AND "recipientId"=$2 AND "deletedBySender"=FALSE)
                 OR ("senderId"=$2 AND "recipientId"=$1 AND "deletedByRecip"=FALSE))
             ORDER BY "createdAt" DESC
             LIMIT $3 OFFSET $4`,
            [req.chatUser.id, req.params.userId, lm, offset]
        );

        // Okundu isaretle
        await dbRun(
            `UPDATE chat_messages SET "isRead" = TRUE, "readAt" = NOW(), "updatedAt" = NOW()
             WHERE "senderId" = $1 AND "recipientId" = $2 AND "isRead" = FALSE`,
            [req.params.userId, req.chatUser.id]
        );

        // Okundu bildirimi gonder (socket)
        if (io && onlineUsers.has(req.params.userId)) {
            for (const sid of onlineUsers.get(req.params.userId)) {
                io.to(sid).emit('chat:message:read', { readBy: req.chatUser.id });
            }
        }

        res.json({ messages: messages.reverse() });
    } catch (err) {
        console.error('[ChatApp Messages]', err.message);
        res.status(500).json({ error: 'Sunucu hatasi' });
    }
});

// DELETE /chat/api/messages/conversations/:partnerId — Sohbeti sil
app.delete('/chat/api/messages/conversations/:partnerId', chatAuthMiddleware, async (req, res) => {
    try {
        const { partnerId } = req.params;
        // Sadece kendi tarafini siler
        await dbRun(
            `UPDATE chat_messages SET "deletedBySender" = TRUE, "updatedAt" = NOW()
             WHERE "senderId" = $1 AND "recipientId" = $2`,
            [req.chatUser.id, partnerId]
        );
        await dbRun(
            `UPDATE chat_messages SET "deletedByRecip" = TRUE, "updatedAt" = NOW()
             WHERE "senderId" = $1 AND "recipientId" = $2`,
            [partnerId, req.chatUser.id]
        );
        res.json({ message: 'Sohbet silindi' });
    } catch (err) {
        res.status(500).json({ error: 'Sunucu hatasi' });
...
// DELETE /chat/api/messages/:id — Tek mesaj sil
app.delete('/chat/api/messages/:id', chatAuthMiddleware, async (req, res) => {
    try {
        const msg = await dbGet(`SELECT * FROM chat_messages WHERE id = $1`, [req.params.id]);
        if (!msg) return res.status(404).json({ error: 'Mesaj bulunamadi' });
        if (msg.senderId !== req.chatUser.id && msg.recipientId !== req.chatUser.id)
            return res.status(403).json({ error: 'Yetkisiz' });

        if (msg.senderId === req.chatUser.id) {
            await dbRun(`UPDATE chat_messages SET "deletedBySender"=TRUE,"updatedAt"=NOW() WHERE id=$1`, [req.params.id]);
        } else {
            await dbRun(`UPDATE chat_messages SET "deletedByRecip"=TRUE,"updatedAt"=NOW() WHERE id=$1`, [req.params.id]);
        }
        res.json({ message: 'Mesaj silindi' });
    } catch (err) {
        res.status(500).json({ error: 'Sunucu hatasi' });
...
// GET /chat/api/messages/:userId/unread-count
app.get('/chat/api/messages/:userId/unread', chatAuthMiddleware, async (req, res) => {
    try {
        const row = await dbGet(
            `SELECT COUNT(*)::int AS count FROM chat_messages
             WHERE "senderId"=$1 AND "recipientId"=$2 AND "isRead"=FALSE AND "deletedByRecip"=FALSE`,
...
// GET /chat/api/messages/total-unread — Toplam okunmamis
app.get('/chat/api/messages/total-unread', chatAuthMiddleware, async (req, res) => {
    try {
        const row = await dbGet(
            `SELECT COUNT(*)::int AS count FROM chat_messages WHERE "recipientId"=$1 AND "isRead"=FALSE AND "deletedByRecip"=FALSE`,
            [req.chatUser.id]
        );
        res.json({ totalUnread: row?.count || 0 });
    } catch (err) {
        res.status(500).json({ error: 'Sunucu hatasi' });
    }
});
...
// =============================================================================
// CHATAPP SOCKET.IO EVENT'LERI — 'chat:' prefix ile mevcut socket'e eklendi
// Mevcut socket event'leriyle (message:send vb.) çakışmaz
// =============================================================================
if (io) {
    // Ayrı namespace kullanmak yerine prefix'li event isimleri — daha temiz
    io.on('connection', (socket) => {
        // Daha önce app socket'inde tanımlanan handler'lar calisiyor
        // ChatApp'e ozel ek event'ler buraya ekliyoruz:

        // chat:message:send — ChatApp'ten gelen mesaj
        socket.on('chat:message:send', async (data) => {
            try {
                const { receiverId, content } = data;
                if (!socket.userId || !receiverId || !content) return;

                // Gonderen chat_users tablosunda mi?
                ...
                if (blocked) { socket.emit('chat:message:error', { error: 'Kullanici engellendi' }); return; }

                const recipient = await dbGet(
                    `SELECT id, username FROM chat_users WHERE id=$1 AND "isActive"=TRUE`,
                    [receiverId]
                );
                if (!recipient) { socket.emit('chat:message:error', { error: 'Alici bulunamadi' }); return; }

                const msgId = uuidv4();
                await dbRun(
                    `INSERT INTO chat_messages (id,"senderId","recipientId","senderUsername","recipientUsername",content,"createdAt","updatedAt")
                     VALUES ($1,$2,$3,$4,$5,$6,NOW(),NOW())`,
                    [msgId, socket.userId, receiverId, sender.username, recipient.username, content.trim().substring(0, 5000)]
                );
                const msg = await dbGet(`SELECT * FROM chat_messages WHERE id=$1`, [msgId]);
                ...
                console.error('[Socket chat:message:send]', e.message);
                socket.emit('chat:message:error', { error: 'Mesaj gonderilemedi' });
            }
        });
        ...
        socket.on('chat:typing', ({ receiverId, isTyping }) => {
            ...
            if (onlineUsers.has(receiverId)) {
                for (const sid of onlineUsers.get(receiverId)) {
                    io.to(sid).emit('chat:typing', { senderId: socket.userId, isTyping: !!isTyping });
                }
            }
        });
        ...
        socket.on('chat:read', async ({ senderId }) => {
            try {
                ...
                if (onlineUsers.has(senderId)) {
                    for (const sid of onlineUsers.get(senderId)) {
                        io.to(sid).emit('chat:message:read', { readBy: socket.userId });
                    }
                }
            } catch (e) { /* sessiz */ }
        });
...
// =============================================================================
// YENİ CHATAPP ROTALARI v9
// =============================================================================

// ── Multer — sadece ChatApp medya yüklemeleri için ──────────────────────────
...
// POST /chat/api/messages/media — Medya mesaj gönder
app.post('/chat/api/messages/media', chatAuthMiddleware, chatMsgLimiter, chatUpload.single('media'), async (req, res) => {
    try {
        const { recipientId, content } = req.body;
        if (!recipientId) return res.status(400).json({ error: 'recipientId gerekli' });
        if (!req.file && !content) return res.status(400).json({ error: 'Medya veya metin gerekli' });
        ...
        const recipient = await dbGet(`SELECT id,username FROM chat_users WHERE id=$1 AND "isActive"=TRUE`, [recipientId]);
        if (!recipient) return res.status(404).json({ error: 'Alici bulunamadi' });
        ...
        await dbRun(
            `INSERT INTO chat_messages (id,"senderId","recipientId","senderUsername","recipientUsername",content,"mediaUrl","mediaType","createdAt","updatedAt")
             VALUES ($1,$2,$3,$4,$5,$6,$7,$8,NOW(),NOW())`,
            [msgId, req.chatUser.id, recipientId, req.chatUser.username, recipient.username,
             (content || '').trim().substring(0, 5000), mediaUrl, mediaType]
        );
        const msg = await dbGet(`SELECT * FROM chat_messages WHERE id=$1`, [msgId]);
        ...
        console.error('[ChatApp MediaMsg]', err.message);
        res.status(500).json({ error: 'Sunucu hatasi' });
    }
});

// POST /chat/api/profile/avatar — Profil fotoğrafı yükle
app.post('/chat/api/profile/avatar', chatAuthMiddleware, chatUpload.single('avatar'), async (req, res) => {
    try {
        if (!req.file) return res.status(400).json({ error: 'Dosya gerekli' });
        const profilePic = `/uploads/chat/${req.file.filename}`;
        await dbRun(`UPDATE chat_users SET "profilePic"=$1,"updatedAt"=NOW() WHERE id=$2`, [profilePic, req.chatUser.id]);
        res.json({ profilePic });
    } catch (err) {
        console.error('[ChatApp Avatar]', err.message);
        res.status(500).json({ error: 'Sunucu hatasi' });
    }
});

// =============================================================================
// CHATAPP GRUP ROTALARI — /chat/api/groups/*
// =============================================================================

// Grup tablolarını oluştur (eğer yoksa)
(async () => {
    try {
        await pool.query(`
            CREATE TABLE IF NOT EXISTS chat_groups (
                ...
                name        TEXT NOT NULL,
                "avatarUrl" TEXT,
                "createdBy" UUID NOT NULL REFERENCES chat_users(id) ON DELETE CASCADE,
                "createdAt" TIMESTAMPTZ NOT NULL DEFAULT NOW(),
                "updatedAt" TIMESTAMPTZ NOT NULL DEFAULT NOW()
            )
        `);
        await pool.query(`
            CREATE TABLE IF NOT EXISTS chat_group_members (
                id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
                "groupId"   UUID NOT NULL REFERENCES chat_groups(id) ON DELETE CASCADE,
                "userId"    UUID NOT NULL REFERENCES chat_users(id) ON DELETE CASCADE,
        ...
        `);
        console.log('✅ ChatApp grup tabloları hazır');
    } catch(e) { console.warn('⚠️ Grup tabloları oluşturulamadı:', e.message); }
})();

// POST /chat/api/groups — Grup oluştur
app.post('/chat/api/groups', chatAuthMiddleware, async (req, res) => {
    try {
        const { name, memberIds } = req.body;
        if (!name || !name.trim()) return res.status(400).json({ error: 'Grup adı gerekli' });
        if (!Array.isArray(memberIds) || memberIds.length < 1)
            return res.status(400).json({ error: 'En az 1 uye gerekli' });

        const groupId = uuidv4();
        await dbRun(
            `INSERT INTO chat_groups (id, name, "createdBy", "createdAt", "updatedAt") VALUES ($1,$2,$3,NOW(),NOW())`,
            [groupId, name.trim().substring(0, 60), req.chatUser.id]
        );
        // Kurucuyu admin olarak ekle
        await dbRun(`INSERT INTO chat_group_members (id,"groupId","userId",role,"joinedAt") VALUES ($1,$2,$3,'admin',NOW())`,
            [uuidv4(), groupId, req.chatUser.id]);
        // Üyeleri ekle
        const uniqueIds = [...new Set(memberIds)].filter(id => id !== req.chatUser.id).slice(0, 49);
        for (const uid of uniqueIds) {
            const exists = await dbGet(`SELECT id FROM chat_users WHERE id=$1 AND "isActive"=TRUE`, [uid]);
            if (exists) {
                await dbRun(`INSERT INTO chat_group_members (id,"groupId","userId","joinedAt") VALUES ($1,$2,$3,NOW()) ON CONFLICT DO NOTHING`,
                    [uuidv4(), groupId, uid]);
            }
        }
        const group = await dbGet(`SELECT * FROM chat_groups WHERE id=$1`, [groupId]);
        res.status(201).json({ group });
    } catch (err) {
        console.error('[ChatApp CreateGroup]', err.message);
        res.status(500).json({ error: 'Sunucu hatasi' });
    }
});

// GET /chat/api/groups — Kullanıcının gruplarını listele
app.get('/chat/api/groups', chatAuthMiddleware, async (req, res) => {
    try {
        const groups = await dbAll(
            `SELECT g.*, (SELECT COUNT(*)::int FROM chat_group_members WHERE "groupId"=g.id) AS "memberCount"
             FROM chat_groups g
             JOIN chat_group_members gm ON gm."groupId"=g.id
             WHERE gm."userId"=$1
...
// GET /chat/api/groups/:id — Grup detay + üyeler
app.get('/chat/api/groups/:id', chatAuthMiddleware, async (req, res) => {
    try {
        const group = await dbGet(`SELECT * FROM chat_groups WHERE id=$1`, [req.params.id]);
        if (!group) return res.status(404).json({ error: 'Grup bulunamadi' });
        const member = await dbGet(`SELECT id FROM chat_group_members WHERE "groupId"=$1 AND "userId"=$2`, [req.params.id, req.chatUser.id]);
        if (!member) return res.status(403).json({ error: 'Bu gruba erisim yetkiniz yok' });
        const members = await dbAll(
            `SELECT cu.id,cu.username,cu.name,cu."profilePic",cu."isOnline",gm.role,gm."joinedAt"
             FROM chat_group_members gm JOIN chat_users cu ON cu.id=gm."userId"
             WHERE gm."groupId"=$1 ORDER BY gm.role DESC, gm."joinedAt"`,
            [req.params.id]
        );
...
// POST /chat/api/groups/:id/messages — Gruba mesaj gönder
app.post('/chat/api/groups/:id/messages', chatAuthMiddleware, chatMsgLimiter, async (req, res) => {
    try {
        const { content } = req.body;
        if (!content || !content.trim()) return res.status(400).json({ error: 'Mesaj gerekli' });
        const member = await dbGet(`SELECT id FROM chat_group_members WHERE "groupId"=$1 AND "userId"=$2`, [req.params.id, req.chatUser.id]);
        if (!member) return res.status(403).json({ error: 'Bu gruba mesaj gonderme yetkiniz yok' });
        const msgId = uuidv4();
        await dbRun(
            `INSERT INTO chat_group_messages (id,"groupId","senderId","senderUsername",content,"createdAt","updatedAt") VALUES ($1,$2,$3,$4,$5,NOW(),NOW())`,
            [msgId, req.params.id, req.chatUser.id, req.chatUser.username, content.trim().substring(0, 5000)]
        );
        const msg = await dbGet(`SELECT * FROM chat_group_messages WHERE id=$1`, [msgId]);
        // Socket ile tüm grup üyelerine gönder
        if (io) {
            const groupMembers = await dbAll(`SELECT "userId" FROM chat_group_members WHERE "groupId"=$1`, [req.params.id]);
            for (const gm of groupMembers) {
                if (gm.userId !== req.chatUser.id && onlineUsers.has(gm.userId)) {
                    for (const sid of onlineUsers.get(gm.userId)) {
                        io.to(sid).emit('chat:group:message', { groupId: req.params.id, message: msg });
                    }
                }
            }
        }
        ...
        console.error('[ChatApp GroupMsg]', err.message);
        res.status(500).json({ error: 'Sunucu hatasi' });
    }
});

// GET /chat/api/groups/:id/messages — Grup mesajları
app.get('/chat/api/groups/:id/messages', chatAuthMiddleware, async (req, res) => {
    try {
        const member = await dbGet(`SELECT id FROM chat_group_members WHERE "groupId"=$1 AND "userId"=$2`, [req.params.id, req.chatUser.id]);
        if (!member) return res.status(403).json({ error: 'Erisim yetkiniz yok' });
        const limit  = Math.min(parseInt(req.query.limit) || 60, 200);
        const before = req.query.before;
        const msgs = await dbAll(
            `SELECT gm.*,cu."profilePic" AS "senderPic" FROM chat_group_messages gm
             JOIN chat_users cu ON cu.id=gm."senderId"
             WHERE gm."groupId"=$1 ${before ? 'AND gm."createdAt" < $3' : ''}
             ORDER BY gm."createdAt" DESC LIMIT $2`,
...
// DELETE /chat/api/groups/:id/leave — Gruptan ayrıl
app.delete('/chat/api/groups/:id/leave', chatAuthMiddleware, async (req, res) => {
    try {
        await dbRun(`DELETE FROM chat_group_members WHERE "groupId"=$1 AND "userId"=$2`, [req.params.id, req.chatUser.id]);
...
// =============================================================================
// CHATAPP ARAMA (CALL) SOCKET EVENTS
// =============================================================================
if (io) {
    io.on('connection', (socket) => {
        // chat:call:start — Arama başlat
        socket.on('chat:call:start', async ({ calleeId }) => {
            if (!socket.userId || !calleeId) return;
            // Engel kontrolu
            const blocked = await dbGet(
                `SELECT id FROM chat_blocks WHERE ("blockerId"=$1 AND "blockedId"=$2) OR ("blockerId"=$2 AND "blockedId"=$1)`,
                [socket.userId, calleeId]
            ).catch(() => null);
            if (blocked) { socket.emit('chat:call:error', { error: 'Bu kullaniciya arama yapamazsiniz' }); return; }

            const caller = await dbGet(`SELECT id,name,username,"profilePic" FROM chat_users WHERE id=$1`, [socket.userId]).catch(() => null);
            if (!caller) return;

            if (onlineUsers.has(calleeId)) {
                for (const sid of onlineUsers.get(calleeId)) {
                    io.to(sid).emit('chat:call:incoming', {
                        callerId: socket.userId,
                        callerName: caller.name || caller.username,
                        callerAvatar: caller.profilePic
                    });
                }
                socket.emit('chat:call:ringing', { calleeId });
            } else {
                socket.emit('chat:call:unavailable', { calleeId });
            }
        });

        // chat:call:accept
        socket.on('chat:call:accept', ({ callerId }) => {
            if (!socket.userId || !callerId) return;
            if (onlineUsers.has(callerId)) {
                for (const sid of onlineUsers.get(callerId)) {
                    io.to(sid).emit('chat:call:accepted', { calleeId: socket.userId });
                }
            }
        });

        // chat:call:decline
        socket.on('chat:call:decline', ({ callerId }) => {
            if (!socket.userId || !callerId) return;
            if (onlineUsers.has(callerId)) {
                for (const sid of onlineUsers.get(callerId)) {
                    io.to(sid).emit('chat:call:declined', { calleeId: socket.userId });
                }
            }
        });

        // chat:call:cancel — Arayandan iptal
        socket.on('chat:call:cancel', ({ calleeId }) => {
            if (!socket.userId || !calleeId) return;
            if (onlineUsers.has(calleeId)) {
                for (const sid of onlineUsers.get(calleeId)) {
                    io.to(sid).emit('chat:call:cancelled');
                }
            }
        });
    });
...
app.get('*', (req, res, next) => {
    // API ve upload istekleri buraya düşmemeli
    if (req.path.startsWith('/api/') || req.path.startsWith('/uploads/')) return next();
    // /chat/* kendi handler'ına zaten yönlendirildi — buraya düşmez
